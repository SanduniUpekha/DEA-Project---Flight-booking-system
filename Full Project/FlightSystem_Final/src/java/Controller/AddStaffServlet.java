...
public class AddStaffServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        management.setFullName(fullName);
        management.setAge(age);
        management.setNic(nic);
        management.setEmail(email);
        management.setUsername(username);
        management.setPassword(password);
        management.setGrade(grade);
        
        if (staffManagement.addStaff(management) == 0) {
            response.sendRedirect("admin/stff.jsp");
        }
    }
